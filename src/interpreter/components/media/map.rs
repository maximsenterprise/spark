...
pub fn map(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<map>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</map>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<map>\n".to_string() + &background + "\t</map>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<map{}>{}</map>", &arguments, &content);
    }
    else {
        return format!("<map{} style=\"{}\">{}</map>", &arguments, &style, &content);
    }
}
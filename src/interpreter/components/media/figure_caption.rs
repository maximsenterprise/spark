...
pub fn figure_caption(nodes: &mut Vec<Node>) -> String {
    ...
    if style == "" {
        title = format!("<figcaption{}>{}</figcaption>", arguments, value);
    }
    else {
        title = format!("<figcaption{} style=\"{}\">{}</figcaption>", arguments, style, value);
    }

...
pub fn footer(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<footer>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</footer>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<footer>\n".to_string() + &background + "\t</footer>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<footer{}>{}</footer>", &arguments, &content);
    }
    else {
        return format!("<footer{} style=\"{}\">{}</footer>", &arguments, &style, &content);
    }
}
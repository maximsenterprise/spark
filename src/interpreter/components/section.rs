...
pub fn section(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<section>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</section>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<section>\n".to_string() + &background + "\t</section>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string();
    if style == "" {
        return format!("<section{}>{}</section>", &arguments, &content);
    }
    else {
        return format!("<section{} style=\"{}\">{}</section>", &arguments, &style, &content);
    }
}
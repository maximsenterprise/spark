...
pub fn content_component(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<content>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</content>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<content>\n".to_string() + &background + "\t</content>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<content{}>{}</content>", &arguments, &content);
    }
    else {
        return format!("<content{} style=\"{}\">{}</content>", &arguments, &style, &content);
    }
}
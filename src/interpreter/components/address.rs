...
pub fn address(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<address>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</address>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<address>\n".to_string() + &background + "\t</address>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<address{}>{}</address>", &arguments, &content);
    }
    else {
        return format!("<address{} style=\"{}\">{}</address>", &arguments, &style, &content);
    }
}
...
pub fn ruby(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<ruby>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</ruby>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<ruby>\n".to_string() + &background + "\t</ruby>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string();
    if style == "" {
        return format!("<ruby{}>{}</ruby>", &arguments, &content);
    }
    else {
        return format!("<ruby{} style=\"{}\">{}</ruby>", &arguments, &style, &content);
    }
}
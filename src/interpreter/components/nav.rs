...
pub fn nav(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<nav>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</nav>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<nav>\n".to_string() + &background + "\t</nav>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<nav{}>{}</nav>", &arguments, &content);
    }
    else {
        return format!("<nav{} style=\"{}\">{}</nav>", &arguments, &style, &content);
    }
}
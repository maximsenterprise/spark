...
pub fn article(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<article>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</article>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<article>\n".to_string() + &background + "\t</article>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<article{}>{}</article>", &arguments, &content);
    }
    else {
        return format!("<article{} style=\"{}\">{}</article>", &arguments, &style, &content);
    }
}
...
pub fn search(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<search>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</search>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<search>\n".to_string() + &background + "\t</search>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<search{}>{}</search>", &arguments, &content);
    }
    else {
        return format!("<search{} style=\"{}\">{}</search>", &arguments, &style, &content);
    }
}
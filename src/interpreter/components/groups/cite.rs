...
pub fn cite(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<cite>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</cite>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<cite>\n".to_string() + &background + "\t</cite>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string();
    if style == "" {
        return format!("<cite{}>{}</cite>", &arguments, &content);
    }
    else {
        return format!("<cite{} style=\"{}\">{}</cite>", &arguments, &style, &content);
    }
}
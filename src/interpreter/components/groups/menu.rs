...
pub fn menu(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<menu>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</menu>";
        }
        else {
            loop {
                if nodes.len() == 0 {
    ...
    if style == "" {
        ...
        return format!("<menu{} style=\"{}\">{}</menu>", &arguments, &style, &content);
    }
}
...
pub fn ol(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<ol>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</ol>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<ol>\n".to_string() + &background + "\t</ol>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<ol{}>{}</ol>", &arguments, &content);
    }
    else {
        return format!("<ol{} style=\"{}\">{}</ol>", &arguments, &style, &content);
    }
}
...
pub fn canvas(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<canvas>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</canvas>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<canvas>\n".to_string() + &background + "\t</canvas>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<canvas{}>{}</canvas>", &arguments, &content);
    }
    else {
        return format!("<canvas{} style=\"{}\">{}</canvas>", &arguments, &style, &content);
    }
}
...
pub fn picture(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<picture>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</picture>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<picture>\n".to_string() + &background + "\t</picture>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string();
    if style == "" {
        return format!("<picture{}>{}</picture>", &arguments, &content);
    }
    else {
        return format!("<picture{} style=\"{}\">{}</picture>", &arguments, &style, &content);
    }
}
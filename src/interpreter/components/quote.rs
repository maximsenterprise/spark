...
pub fn quote(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<quote>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</quote>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<quote>\n".to_string() + &background + "\t</quote>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<quote{}>{}</quote>", &arguments, &content);
    }
    else {
        return format!("<quote{} style=\"{}\">{}</quote>", &arguments, &style, &content);
    }
}
...
pub fn div(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<div>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</div>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<div>\n".to_string() + &background + "\t</div>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<div{}>{}</div>", &arguments, &content);
    }
    else {
        return format!("<div{} style=\"{}\">{}</div>", &arguments, &style, &content);
    }
}
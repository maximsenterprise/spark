...
pub fn header(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<header>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</header>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<header>\n".to_string() + &background + "\t</header>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<header{}>{}</header>", &arguments, &content);
    }
    else {
        return format!("<header{} style=\"{}\">{}</header>", &arguments, &style, &content);
    }
}
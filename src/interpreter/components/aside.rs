...
pub fn aside(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<aside>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</aside>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<aside>\n".to_string() + &background + "\t</aside>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string();
    if style == "" {
        return format!("<aside{}>{}</aside>", &arguments, &content);
    }
    else {
        return format!("<aside{} style=\"{}\">{}</aside>", &arguments, &style, &content);
    }
}
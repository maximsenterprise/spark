...
pub fn pre(nodes: &mut Vec<Node>) -> String {
        ...
        Node::Other(other) => {
            if other.token.token_type == TokenType::Comma {
                ...
                loop {
                    ...
                    match nodes[0].clone() {
                        Node::Literal(preteral) => {
                            let aspect_name = preteral.value;
                            eat(nodes);
    ...
    if style == "" {
        title = format!("<pre{}>{}</pre>", arguments, value);
    }
    else {
        title = format!("<pre{} style=\"{}\">{}</pre>", arguments, style, value);
    }

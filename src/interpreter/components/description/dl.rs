...
pub fn dl(nodes: &mut Vec<Node>, add_to_dom: bool) -> String {
    ...
    if add_to_dom == true {
        ...
        if nodes.len() == 0 {
            return "<dl>\n".to_string() + INNER_HTML.lock().unwrap().clone().as_str() + "\t</dl>";
        }
        else {
            loop {
                if nodes.len() == 0 {
        ...
        return "<dl>\n".to_string() + &background + "\t</dl>";
    }

    
    let content = INNER_HTML.lock().unwrap().to_string(); INNER_HTML.lock().unwrap().clear();
    if style == "" {
        return format!("<dl{}>{}</dl>", &arguments, &content);
    }
    else {
        return format!("<dl{} style=\"{}\">{}</dl>", &arguments, &style, &content);
    }
}